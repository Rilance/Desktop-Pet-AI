...
def call_api(input_text):
    ...
    data = {
        "model": "Llama3-TAIDE-LX-8B-Chat-Alpha1.Q6_K.gguf",
    ...
    def update_bubble(self, response_text):
        if response_text.strip():
            ...
            for char in response_text:
                ...
                QApplication.processEvents()  # Refresh the interface to display text character by character
                time.sleep(0.05)  # Control the speed of character-by-character display (can be adjusted as needed)
            self.bubble_label.adjustSize()
            threading.Thread(target=tts_thread, args=(response_text, language)).start()  # Only call TTS once after the full text is displayed
        else:
            self.hide()
